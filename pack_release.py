...
EXCLUDE_DIRS = {
    ...
    "logs", "uploads-test", ".gocache", ".gomodcache",
}
EXCLUDE_FILE_PATTERNS = {"*.pyc", "tmp_*.db", "*_regression.db", ".env"}
...
def go_env(go_dir: Path) -> dict:
    ...
    mod_cache_dir = go_dir / ".gomodcache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    mod_cache_dir.mkdir(parents=True, exist_ok=True)
    env.setdefault("GOCACHE", str(cache_dir.resolve()))
    env.setdefault("GOMODCACHE", str(mod_cache_dir.resolve()))
...
def go_command() -> str | None:
    """Return a usable Go command even when the current terminal PATH is stale."""
    found = shutil.which("go")
    if found:
        return found
    default_windows_go = Path(r"C:\Program Files\Go\bin\go.exe")
    if default_windows_go.exists():
        return str(default_windows_go)
    return None
...
def build_go_backend() -> bool:
    ...
    go_cmd = go_command()
    if not go_cmd:
        print("  [ERROR] Go not found. Install Go 1.22+ and reopen the terminal.")
        print("  Windows installer: https://go.dev/dl/")
        print("  After install, verify with: go version")
        return False
