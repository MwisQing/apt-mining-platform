...
| 2026-05-25 | v4.18 研判工作台 UI 布局优化 | **修复 3 项 UI 布局问题**：(1) **垂直滚动条贴最右边** — `Workbench.vue` 改为 CSS Grid 两行布局（`grid-template-rows: auto 1fr`），筛选栏固定顶部，表格区域占满剩余空间独立滚动（`overflow-y: auto`）；`App.vue` 的 `.content-pane` 移除 `overflow: auto`，滚动由 workbench 内部处理，垂直滚动条出现在页面最右边；(2) **左侧导航收拢至 64px 图标模式** — `App.vue` 新增 `sidebarCollapsed` 状态，侧栏顶部新增收拢/展开按钮，收拢时 `grid-template-columns` 从 `240px` 切换为 `64px`，品牌文字/底部导航文字隐藏，只保留图标（`v-show="!sidebarCollapsed"`），状态保存到 `localStorage`，过渡动画 300ms；(3) **水平滚动条顶部可见** — 表格上方新增 `.table-scroll-h-bar`（高度 14px），通过 `ResizeObserver` 监听表格宽度变化，`scrollLeft` 双向同步（顶部滚动条 ↔ 表格），仅内容溢出时显示；涉及文件：`App.vue`（侧栏收拢+grid切换）、`Workbench.vue`（CSS Grid布局+水平滚动同步）、`global.css` 未修改 |
| 2026-05-25 | v4.19 UI 细节修复 | **修复 4 项 UI 细节问题**：(1) **滚动条变细** — `global.css` `::-webkit-scrollbar` width/height 从 `28px` 改回标准 `10px`；(2) **侧栏收拢按钮重叠修复** — `App.vue` 侧栏切换按钮从 `position: absolute` 改为 flex 子元素（`align-self: flex-end`），不再与导航项重叠；增加 `font-weight: 700` 图标加粗；收拢状态下 `align-self: center` 居中；(3) **工作台右侧滚动条消除** — `App.vue` `.main-shell` 增加 `overflow: hidden`，阻止页面级滚动条，仅表格区域内部滚动；(4) **候选清单表头固定（Excel风格）** — `Workbench.vue` `.candidates-table :deep(.el-table__header-wrapper)` 设置 `position: sticky; top: 0; z-index: 10`，上下滚动时表头固定不动；水平滚动条高度同步从 14px 改为 10px |
