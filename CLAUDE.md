...
| 2026-05-08 | ??30 | ?????????+??ID hash ??????? Workbench ? `analysis_status` / `is_focused` ???????????????????? `source_ip` / `analysis_status` / `is_focused` ???`extract_iocs_from_text` ?????????ID / device id???????????? hash????????????? `MD5` / `SHA1` ???????? 3 ?????????? `python -m unittest backend.tests.test_event_extraction backend.tests.test_workbench_sort_columns backend.tests.test_candidate_sorting backend.tests.test_filter_options` ? `npm run build` ???? |
