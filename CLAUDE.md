...
| 2026-05-25 | v4.15 候选查询去重修复+分页稳定性 | **修复日期筛选后返回行数不足（10w → 4w）+ 分页重叠**：(1) **根因1** — `candidate_repo.go` `base` CTE 使用 `ROW_NUMBER() OVER (PARTITION BY device_id, target, port)` 去重，每个组合只保留 1 行，导致大量数据被丢弃；同时 COUNT 查询不走去重，返回 total=10w 但实际只返回 4w 行；(2) **根因2** — `events` 和 `traced` CTE 中同一 target 有多条记录时，LEFT JOIN 产生行扇出（同一 alert ID 在分页中出现多次）；(3) **根因3** — ORDER BY 仅按 `candidate_score` 排序，121 行同分时 PostgreSQL 无确定性排序，OFFSET 分页跨页重叠；(4) **修复** — `base` CTE 改为 `SELECT a.* FROM alerts a WHERE %s` 移除去重；`events` CTE 改为 `ROW_NUMBER() OVER (PARTITION BY UPPER(target), port ORDER BY me.id)` 去重；`traced` CTE 改为 `ROW_NUMBER() OVER (PARTITION BY UPPER(target), port ORDER BY traced_at DESC)` 去重；ORDER BY 改为 `%s %s, id %s` 加入 id 作为次级排序键；COUNT 查询参数改为 `args[:nWhereArgs]` 精确截取；(5) **验收 15/15 PASS**：total=12552 一致、1000 行无重复 ID、分页无重叠、日期边界正确、3 项排序回归通过、组合筛选无报错 |
...
| 2026-05-29 | v4.20 多 Excel 导入进度与轮询修复 | **修复多 Excel 上传看似卡住、快照状态轮询过密的问题**：(1) **根因** — 导入队列本身为单 worker 串行处理，多个文件会排队；但后端处理过程中只在完成时写回 `imports` 统计，前端长时间只能显示”正在读取文件”；同时 `loadSnapshotStatus()` 递归 `setTimeout` 会叠加轮询，`/api/snapshots/status` 每次执行 `COUNT(*) FROM alerts`，导入写入时会额外占用数据库资源；(2) **后端修复** — `import_service.go` 新增 `importProgress`，每 500 行按已处理行更新 `parsed_rows/rows_inserted/rows_skipped/raw_rows/rows_failed`，处理中不再把 `total_rows` 误当作已处理数；补齐 `import_rows` 必填字段 `source_file/created_at`，缺字段行写入 `raw_only`，最终状态按 `success/partial/failed` 返回；`reprocess-queued` 兼容返回 `requeued`；(3) **前端修复** — `Settings.vue` 优先展示真正 processing 的任务，进度优先读 `parsed_rows`；快照状态改为单一 15 秒轮询，完成检查中 `status=ready` 即视为就绪，避免一直等待不存在的 `active_version`；(4) **测试** — 新增 `import_progress_test.go` 覆盖导入最终状态与已处理行计算；验证 `go test ./...`、`go build ./...`、`npm run build` 通过，前端构建产物已同步至 `backend_v2/static/`；当前仓库未包含 `scripts/test_api.py`，无法执行该脚本 |
| 2026-05-29 | v4.20 导入批 INSERT 修复 | **修复 v4.20 导入直接失败**：(1) **根因1** — `flushBatch` 批量 INSERT 使用 `SELECT * FROM (SELECT unnest...) AS t WHERE NOT EXISTS (...t.content_hash)` 但子查询 `t` 无命名列别名，PostgreSQL 无法解析 `t.content_hash`；(2) **根因2** — `first_alert_time`/`last_alert_time` 的 `unnest($N::text[])` 产生 text 值，INSERT 目标列为 timestamp，无显式 cast 导致类型不匹配 (42804)；(3) **根因3** — `beginImportBatch` 单行回退 INSERT 的 asset_type/source_file、import_id/import_sheet_id 列顺序与批量 INSERT 相反；(4) **修复** — 移除子查询包装，改为 Go 层预查已存在 content_hash（`WHERE content_hash = ANY(...)`），在 Go 中 dedup 后再批量 INSERT；时间字段加 `::timestamp` 显式转换；移除死代码 fallback；`import_sheets` 的 `parsed_rows` 修正为 `sheetRows-sheetRaw-sheetFailed` 避免膨胀；(5) **验收 8/8 通过**：上传 10 行测试数据全部成功插入（status=success）、source_ip/target/port 列数据正确、关键词搜索匹配、候选查询正常、事件/标签/健康检查无回归 |
| 2026-05-29 | 发布脚本排除 uploads | **pack/push 脚本排除 `backend_v2/uploads`**：(1) `pack_release.py` — 删除旧 `copy_ignore_func`（shutil.copytree 回调），替换为 `copy_tree_with_skip()` 递归复制，新增 `src.name == "backend_v2" and item.name == "uploads"` 跳过条件；(2) `push_release.py` — `GIT_EXCLUDE` 追加 `"backend_v2/uploads/"`，`git reset HEAD --` 自动解除暂存；(3) **测试发现并修复** — `VERSION` 复制条件 `src.name != ""` 永远为真（死代码），根 VERSION 不被复制；改为 `is_root` 布尔参数递归传递，根目录 `is_root=True` 复制 VERSION，子目录 `is_root=False` 跳过；(4) **运行报错修复** — `copy_project` 调用 `copy_tree_with_skip` 前未 `dest.mkdir(exist_ok=True)`，顶层目录不存在导致 WinError 3（原 `shutil.copytree` 自动创建目标目录，替换为递归函数后遗漏）；(5) **验收 7/7 PASS**：`backend_v2/uploads` 排除、正常目录仍复制、根 VERSION 复制、data 排除、pyc 排除、git reset 语法有效、真实项目树集成测试通过 |
| 2026-06-01 | v4.21 工作台事件生成体验修复 | **优化研判工作台 IOC 一键生成事件与表格横向滚动体验**：(1) `GET /api/alert-candidates/ioc-devices` 新增按当前日期范围 + IOC + 端口查询设备 ID 与设备标签，弹窗不受当前表格筛选/分页影响；(2) `Workbench.vue` 一键生成事件默认标题改为 `银狐-当前日期`，Text 模板改为“线索ioc / 设备id / 标题 / 分隔线”格式并允许用户临时编辑；(3) 设备标签改为多选下拉，选择标签时自动向 Text 模板新增该标签命中的设备 ID，取消标签时只移除由该标签新增的设备 ID，保留当前设备和用户手动编辑内容；(4) 保留“仅当前设备”按钮用于快速恢复单设备模板；(5) 提交事件后不再主动 `loadData()` 刷新候选列表，仅在本地标记同 IOC 行事件状态，避免清空用户筛选、分页和当前行数；(6) 表格 `el-table` 设置固定高度，横向滚动条固定在候选表区域底部显示；(7) 前端构建产物已同步至 `backend_v2/static/`；**验证**：`node frontend/src/utils/eventText.test.mjs`、`go test ./...`、`go build ./...`、`npm run build` 通过；浏览器插件本轮不可用，未执行真实浏览器点击烟测 |
| 2026-06-09 | init_db.bat PostgreSQL 初始化修复 | **修复 Windows 初始化脚本可用性问题**：(1) `PG_BIN` 默认改为 `psql`，支持 `.env` 或环境变量覆盖，不再硬编码 PostgreSQL 18 路径；(2) `APT_DB_HOST`/`APT_DB_PORT`/`APT_DB_USER`/`APT_DB_PASSWORD`/`APT_DB_NAME` 全部从 `.env` 读取，缺省值与 `start.py` 保持一致；(3) 允许空密码，兼容本地 trust 认证；(4) 数据库存在判断改为读取 `psql -tAc` 查询结果，避免查询空结果时误判为已存在；(5) 新增可选 `APT_DB_ADMIN_USER`/`APT_DB_ADMIN_PASSWORD` 支持建库账号和业务账号分离；(6) 所有 `psql` 调用使用 `call`，兼容 `PG_BIN` 指向 `.bat/.cmd` 包装器；(7) 末尾启动提示改为当前可用入口 `python start.py`；**验证**：新增 `tests/test_init_db_bat.py` 静态回归和 `tests/test_init_db_bat_acceptance.py` 临时 fake psql 验收，`python -m unittest tests.test_init_db_bat` 5/5 通过，`python -m unittest tests.test_init_db_bat_acceptance` 2/2 通过；全量 `python -m unittest discover tests` 仍受既有 `psycopg2` 缺失影响，`test_ops_data_go_scripts` 3 项 import 阶段报错，非本次修改引入。 |
| 2026-06-09 | 本机 PostgreSQL 18 初始化验收 | **完成本机数据库初始化前置配置和验证**：(1) 用户已安装 PostgreSQL 18.4，确认 `C:\Program Files\PostgreSQL\18\bin\psql.exe` 可用；(2) 根目录 `.env` 补充 `PG_BIN=C:\Program Files\PostgreSQL\18\bin\psql.exe`，解决 `psql was not found`；(3) 使用现有 `.env` 中 `APT_DB_USER=apt_test`、`APT_DB_NAME=apt_mining_test` 执行 `init_db.bat` 成功，脚本确认/创建 14 张表：`alerts`、`audit_log`、`config`、`device_tags`、`event_followups`、`import_rows`、`import_sheets`、`imports`、`mined_event_devices`、`mined_event_iocs`、`mined_events`、`tag_batches`、`tags`、`traced_targets`；(4) Python 环境验证 `psycopg2` 可导入，`import_ops_data_go.py` 具备运行依赖；**后续操作**：可执行 `python import_ops_data_go.py ops-export-go.json` 导入 Go/PostgreSQL 运营数据包，输入文件路径按实际导出文件替换。 |
| 2026-06-09 | pack_release.py Go 缺失提示修复 | **修复一键打包在缺少 Go 编译器时直接抛 `FileNotFoundError` 的问题**：(1) 根因确认：当前机器 `where go` 失败，`C:\Program Files\Go\bin\go.exe` 不存在，`pack_release.py` 在 `go mod download` 子进程创建阶段崩溃；(2) `build_go_backend()` 对 `go mod download`、`go build` 和交叉编译调用补充 `FileNotFoundError` 捕获，输出 Go 1.22+ 安装、重开终端和 `go version` 验证提示；(3) 新增 `tests/test_pack_release.py`，覆盖 Go 缺失时返回 `False` 不抛异常，以及 Go 构建命令沿用仓库内 `backend_v2/.gocache`；**验证**：`python -m unittest tests.test_pack_release` 2/2 通过；当前机器直接调用 `pack_release.build_go_backend()` 返回缺 Go 的明确提示，未再出现 Python traceback。 |
| 2026-06-09 | pack_release.py Go 代理与缓存修复 | **修复 Go 依赖下载卡住/权限失败问题**：(1) `pack_release.py` 的 Go 构建环境新增 `GOPROXY=https://goproxy.cn,direct`、`GOSUMDB=sum.golang.google.cn`，并默认使用用户本地代理 `HTTP_PROXY`/`HTTPS_PROXY=http://127.0.0.1:10809`；(2) 新增 `GOMODCACHE=backend_v2/.gomodcache` 且使用绝对路径，避免默认写入 `C:\Users\Anya\go\pkg\mod\cache` 时触发 `Access is denied`；(3) `go_command()` 支持当前终端 PATH 未刷新时自动使用 `C:\Program Files\Go\bin\go.exe`；(4) 打包复制排除 `.gomodcache`，避免把模块缓存放入发布包；(5) `tests/test_pack_release.py` 补充代理和模块缓存断言；**验证**：`python -m unittest tests.test_pack_release` 2/2 通过，真实 `go mod download` 通过，真实 `pack_release.build_go_backend()` 通过。 |
| 2026-06-09 | 发布脚本敏感文件与 Go 缓存排除 | **补全打包/推送排除规则**：(1) `pack_release.py` 的 `EXCLUDE_DIRS` 加入 `.gocache` 和 `.gomodcache`，`EXCLUDE_FILE_PATTERNS` 加入 `.env`，避免发布包包含 Go 构建/模块缓存和本机数据库密码配置；(2) `push_release.py` 的 `GIT_EXCLUDE` 补充 `.gocache/`、`.gomodcache/`、`backend_v2/.gocache/`、`backend_v2/.gomodcache/` 和 `.env`，避免一键推送误暂存本地缓存/配置；(3) 新增 `tests/test_push_release.py`，`tests/test_pack_release.py` 补充复制行为断言；**验证**：`python -m unittest tests.test_pack_release tests.test_push_release` 4/4 通过，真实 `pack_release.copy_project()` smoke test 确认 `.env`、`backend_v2/.gocache`、`backend_v2/.gomodcache` 未进入复制结果。 |
