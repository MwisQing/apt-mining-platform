...
| 2026-05-26 | v4.18 研判工作台筛选栏收起/展开 | **新增筛选栏收起展开功能**：(1) `Workbench.vue` 新增 `filterCollapsed` ref 控制状态（默认展开），`toggleFilter()` 切换；(2) 筛选栏右下角新增 32px 圆形悬浮按钮，内含 `ArrowUpBold` 图标，收起时旋转 180°；(3) CSS 使用 `cubic-bezier(0.4, 0, 0.2, 1)` 缓动，0.35s 内平滑过渡 max-height/padding/opacity/border；(4) 按钮 hover 变为 accent 色并放大，click 缩小反馈；(5) 前端构建 + 同步至 `backend_v2/static/` |
