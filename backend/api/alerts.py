    ...
    *,
    ...
    if keyword:
        ...
        conditions.append(
            ...
            "OR a.threat_type LIKE :kw OR a.apt_org LIKE :kw OR a.intel_tags LIKE :kw "
            "OR a.vendors LIKE :kw)"
        )
...
def _event_maps_for_rows(db, rows):
    ...
    if device_ids:
        ...
        device_rows = db.execute(
            text(
                ...
                WHERE med.device_id IN ({placeholders})
                """
            ),
        ...
        for row in device_rows:
            ...
            device_map[row[0]] = _prefer_event(device_map.get(row[0]), event_info)
    ...
    return device_map, ioc_exact_map, ioc_wildcard_map
...
def _event_for_row(row_dict, device_map, ioc_exact_map, ioc_wildcard_map):
    port = row_dict.get("port") or ""
    return (
        device_map.get(row_dict.get("device_id"))
        or ioc_exact_map.get((row_dict.get("target"), port))
...
def _query_items(db, where, params, page, page_size, badges_filter=None):
    ...
    if badges_filter:
        all_rows = db.execute(
            text(
                ...
                WHERE {where}
                ORDER BY a.first_alert_time DESC
                """
            ),
            params,
        ).fetchall()
...
def _candidate_sort_value(item, sort_field):
    ...
    if sort_field in {"device_id", "target", "port", "threat_type", "std_apt_org"}:
...
def _source_ip_maps_for_rows(db, base_where, base_params, rows):
    ...
    key_clauses = []
    key_params = dict(base_params)
    for i, (device_id, target, port) in enumerate(keys):
        key_clauses.append(
            f"(a.device_id = :sip_did_{i} AND a.target = :sip_tgt_{i} AND COALESCE(a.port, '') = :sip_prt_{i})"
        )
        key_params[f"sip_did_{i}"] = device_id
        key_params[f"sip_tgt_{i}"] = target
        key_params[f"sip_prt_{i}"] = port

    key_where = " OR ".join(key_clauses)
    result_rows = db.execute(
        text(
            ...
            FROM alerts a
            WHERE ({base_where}) AND ({key_where})
            GROUP BY a.device_id, a.target, COALESCE(a.port, '')
            """
        ),
        key_params,
    ...
    for row in result_rows:
        raw_ips = row[3] or ""
        source_ip_map[(row[0], row[1], row[2])] = {
...
def _query_candidate_items(
    ...
    candidate_where, candidate_params = _candidate_filter_sql()
    where = f"({base_where}) AND {candidate_where}"
    params = dict(base_params)
...
def query_alerts(
    ...
    items, total = _query_items(db, where, params, page, page_size, badges_filter)
