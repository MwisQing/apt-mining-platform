...
def _resolve_tag_filter_ids(db, value):
    ...
    if names:
        placeholders = ", ".join(f":tn_{i}" for i in range(len(names)))
        tag_rows = db.execute(
            text(f"SELECT id FROM tags WHERE name IN ({placeholders})"),
            {f"tn_{i}": name for i, name in enumerate(names)},
        ).fetchall()
        ids.extend(int(row[0]) for row in tag_rows)

    unique_ids = sorted(set(ids))
    return ",".join(str(tag_id) for tag_id in unique_ids) if unique_ids else "0"
...
def _build_where(
    *,
    ...
    apt_org_names = _csv_values(apt_orgs)
    if apt_org_names:
        placeholders = ", ".join(f":ao_{index}" for index in range(len(apt_org_names)))
        conditions.append(f"a.std_apt_org IN ({placeholders})")
        for index, name in enumerate(apt_org_names):
            params[f"ao_{index}"] = name

    port_list = _csv_values(ports)
    if port_list:
        placeholders = ", ".join(f":pt_{index}" for index in range(len(port_list)))
        conditions.append(f"a.port IN ({placeholders})")
        for index, port in enumerate(port_list):
            params[f"pt_{index}"] = port
    ...
    if tag_ids:
        ...
        conditions.append(
            ...
            f"  WHERE UPPER(dt.device_id) = UPPER(a.device_id) AND dt.tag_id IN ({placeholders})"
            ")"
        )
...
def _event_maps_for_rows(db, rows):
    """Build IOC→event and device→event maps.

    IOC maps: matched by target+port.
    Device map: matched by device_id (only for events that also have IOC entries).
    """
    ...
    # Build device→events map (each device keeps ALL events with their IOC targets)
    # Key: uppercased device_id, Value: list of {event_info, ioc_targets: set of (target, port)}
    # Uses UPPER() for case-insensitive matching (event creation uppercases device IDs
    # via _ordered_unique_upper, but alert rows preserve original case from Excel).
    device_events_map = {}
    if device_ids:
            ...
            dev_rows = db.execute(
                text(
                    ...
                    SELECT
                        ...
                        mei.target,
                        COALESCE(mei.port, '') AS port
                    FROM mined_event_devices med
                    JOIN mined_events me ON me.id = med.event_id
                    JOIN mined_event_iocs mei ON mei.event_id = me.id
                    WHERE UPPER(med.device_id) IN ({placeholders})
                    """
                ),
                {f"device_{index}": value.upper() for index, value in enumerate(chunk)},
            ).fetchall()
            for row in dev_rows:
                device_id_key = (row[0] or "").upper()
                event_id = row[1]
                event_info = {
                    "event_id": row[1],
                    "event_name": row[2],
                    "color": row[3],
                    "status": row[4],
                    "mined_at": str(row[5]) if row[5] else None,
                }
                ioc_target = (row[6] or "", row[7] or "")
                entry = None
                for e in device_events_map.setdefault(device_id_key, []):
                    if e["event_info"]["event_id"] == event_id:
                        entry = e
                        break
                if entry is None:
                    entry = {"event_info": event_info, "ioc_targets": set()}
                    device_events_map[device_id_key].append(entry)
                entry["ioc_targets"].add(ioc_target)

    return device_events_map, ioc_exact_map, ioc_wildcard_map
...
def _make_items(db, rows, cross_day_pairs, lateral_ips, badges_filter=None, trace_index=None):
    ...
    for row in rows:
        ...
        event_info = _event_for_row(row_dict, device_events_map, ioc_exact_map, ioc_wildcard_map)
        # device_event: same event must match BOTH this row's IOC (target+port) AND device_id
        device_event = None
        device_id = row_dict.get("device_id")
        target = row_dict.get("target") or ""
        port = row_dict.get("port") or ""
        row_ioc_key = (target, port)
        for entry in device_events_map.get((device_id or "").upper(), []):
            if row_ioc_key in entry["ioc_targets"] or (target, "") in entry["ioc_targets"]:
                device_event = entry["event_info"]
                break
        ...
        row_dict["device_event"] = device_event
...
def _heat_and_source_maps(db, base_where, base_params, rows):
    ...
    for row in agg_rows:
        ...
        device_target_counts[device_id] = device_target_counts.get(device_id, 0) + count
...
def _decorate_candidate_items(db, base_where, base_params, rows, badges_filter=None, target_kind=None, sort_field=None, sort_direction="DESC"):
    ...
    if badges_filter:
        decorated = _filter_by_badges(decorated, badges_filter)
...
HEAT_SORT_JOINS = {
    ...
    "device_target_count": (
        "LEFT JOIN (SELECT device_id, COUNT(*) AS cnt FROM alerts GROUP BY device_id) hdt ON a.device_id = hdt.device_id",
        "COALESCE(hdt.cnt, 0)",
    ),
}
...
def _query_all_candidate_items(
    ...
    rows = db.execute(
        text(f"SELECT a.* FROM alerts a WHERE {where} ORDER BY a.first_alert_time DESC, a.id DESC"),
        params,
    ).fetchall()
...
def _query_candidate_items(
    ...
    if can_page_before_decorating:
        ...
        rows_sql = _ROW_WRAP.format(extra_joins=extra_joins, extra_select=extra_select, where=where)
        rows = db.execute(
            text(
                f"""
                {rows_sql}
    ...
    # Fallback: fetch all rows and sort in Python (for badge-filtered or target_kind-filtered with computed sort)
    rows_sql = _ROW_WRAP.format(extra_joins=extra_joins, extra_select=extra_select, where=where)
...
def _snapshot_row_to_response(snap):
    """Convert a snapshot table row dict to API response format. Pure field mapping, no computation."""
    return {
        "id": snap["id"],
        ...
        "candidate_priority": {
            "id": snap.get("candidate_priority"),
            "label": snap.get("candidate_priority_label"),
            "rank": snap.get("sort_priority_rank", 0),
        },
        "candidate_priority_label": snap.get("candidate_priority_label"),
        "heat": {
        ...
        "trace_status_label": {
            "active": "追踪 TTL 内",
            "expired": "历史追踪",
        }.get(snap.get("trace_status"), "未追踪"),
        "script_bucket": snap.get("target_kind") if snap.get("target_kind") in {"ip", "domain"} else "other",
        "sort_signals": {
            "priority_rank": snap.get("sort_priority_rank", 0),
            "rule_hits": snap.get("sort_rule_hits", 0),
            "target_device_count": snap.get("sort_target_device_count", 0),
            "target_alert_count": snap.get("sort_target_alert_count", 0),
            "source_ip_alert_count": snap.get("sort_source_ip_alert_count", 0),
            "trace_status": snap.get("sort_trace_status"),
            "event_status": snap.get("sort_event_status"),
        },
    }


def _refresh_live_relations_for_snapshot_items(db, items):
    """Overlay mutable workbench state onto snapshot rows.

    Snapshot rows are excellent for static candidate fields, but event/tag/trace
    data is edited by analysts and must reflect the live business tables even if
    an incremental snapshot patch missed a row.
    """
    if not items:
        return items
    ...
    device_events_map, ioc_exact_map, ioc_wildcard_map = _event_maps_for_rows(db, relation_rows)
    trace_exact_map, trace_wildcard_map = _trace_maps_for_rows(db, relation_rows)
    tag_map = _device_tag_map_for_rows(db, relation_rows)
    trace_index = _preload_trace_index(db)

    for item in items:
        event_info = _event_for_row(item, device_events_map, ioc_exact_map, ioc_wildcard_map)
        ...
        row_ioc_key = (target, port)
        device_event = None
        for entry in device_events_map.get(device_id.upper(), []):
            if row_ioc_key in entry["ioc_targets"] or (target, "") in entry["ioc_targets"]:
                device_event = entry["event_info"]
                break

        trace_info = _trace_info_for_row(item, trace_exact_map, trace_wildcard_map)
        device_tags = tag_map.get(item.get("device_id"), [])
        heat = item.get("heat") or {}
        matches = detect_candidate_matches(item)
        existing_badge_names = {badge.get("name") for badge in item.get("badges") or []}
        cross_day_pairs = {(item.get("source_ip"), target)} if "cross_day" in existing_badge_names else set()
        lateral_ips = {item.get("source_ip")} if "lateral" in existing_badge_names else set()

        item["event"] = event_info
        item["device_event"] = device_event
        item["event_status"] = event_info.get("status") if event_info else None
        item["traced"] = trace_info
        item["trace_status"] = (
            "active"
            if trace_info and trace_info.get("active")
            else "expired"
        ...
        item["device_tags"] = device_tags
        item["device_note_summary"] = " | ".join(tag["name"] for tag in device_tags)
        item["badges"] = _dedupe_badges(
            compute_badges(
                _fake_row(item),
                cross_day_pairs,
                lateral_ips,
            ...
            + _build_relation_badges(item, device_tags, event_info, trace_info)
        )
        item["relation_summary"] = " | ".join(
            badge["label"] for badge in _build_relation_badges(item, device_tags, event_info, trace_info)
        )
        item["candidate_rule_ids"] = [rule["id"] for rule in matches]
        item["candidate_reasons"] = build_candidate_reason_labels(
            item,
        ...
        item["candidate_priority_label"] = priority["label"]
        item["candidate_summary"] = " | ".join(item["candidate_reasons"][:4])
        sort_signals = dict(item.get("sort_signals") or {})
        sort_signals.update(
            {
                "priority_rank": priority["rank"],
                "rule_hits": len(matches),
        ...
        item["sort_signals"] = sort_signals
...
def _query_from_snapshot(
    ...
    *,
    ...
    """Query candidate data from precomputed snapshot table. Pure SQL, no Python computation."""
    ...
    if date_start:
        ...
        params["date_start"] = f"{date_start} 00:00:00"
    if date_end:
        conditions.append("s.first_alert_time <= :date_end_end")
        params["date_end_end"] = f"{date_end} 23:59:59"
    if target_type:
    ...
    if selected_threat_types:
        clauses = []
        for idx, item in enumerate(selected_threat_types):
            key = f"tt_{idx}"
    ...
    levels = _csv_values(threat_levels)
    if levels:
        placeholders = ", ".join(f":tl_{idx}" for idx in range(len(levels)))
        conditions.append(f"s.threat_level IN ({placeholders})")
        for idx, level in enumerate(levels):
            params[f"tl_{idx}"] = level

    tiers = _csv_values(apt_tiers)
    if tiers:
        placeholders = ", ".join(f":at_{idx}" for idx in range(len(tiers)))
        conditions.append(f"s.apt_org_tier IN ({placeholders})")
        for idx, tier in enumerate(tiers):
            params[f"at_{idx}"] = tier

    apt_org_names = _csv_values(apt_orgs)
    if apt_org_names:
        placeholders = ", ".join(f":ao_{idx}" for idx in range(len(apt_org_names)))
        conditions.append(f"s.std_apt_org IN ({placeholders})")
        for idx, name in enumerate(apt_org_names):
            params[f"ao_{idx}"] = name

    port_list = _csv_values(ports)
    if port_list:
        placeholders = ", ".join(f":pt_{idx}" for idx in range(len(port_list)))
        conditions.append(f"s.port IN ({placeholders})")
        for idx, port in enumerate(port_list):
            params[f"pt_{idx}"] = port

    # device_tags: filter against live tag tables to match the real-time
    # relation overlay used for rendering rows.
    tag_ids = _csv_values(device_tags)
    if tag_ids:
        placeholders = ", ".join(f":tag_{idx}" for idx in range(len(tag_ids)))
        conditions.append(
            "EXISTS ("
            "SELECT 1 FROM device_tags dt "
            "JOIN tags t ON t.id = dt.tag_id "
            "LEFT JOIN tag_batches tb ON tb.id = t.batch_id "
            "WHERE UPPER(dt.device_id) = UPPER(s.device_id) "
            f"AND dt.tag_id IN ({placeholders}) "
            "AND (t.batch_id IS NULL OR tb.status IS NULL OR tb.status != 'deleted')"
            ")"
        )
        for idx, tid in enumerate(tag_ids):
            params[f"tag_{idx}"] = int(tid)

    # exclude_device_tags: same live-table semantics as include filtering.
    etag_ids = _csv_values(exclude_device_tags)
    if etag_ids:
        placeholders = ", ".join(f":et_{idx}" for idx in range(len(etag_ids)))
        conditions.append(
            "NOT EXISTS ("
            "SELECT 1 FROM device_tags dt "
            "JOIN tags t ON t.id = dt.tag_id "
        ...
        for idx, tid in enumerate(etag_ids):
            params[f"et_{idx}"] = int(tid)

    # badges_filter: EXISTS subquery on badge subtable
    ...
    if badge_names:
        for idx, name in enumerate(badge_names):
            conditions.append(
                ...
                f"AND sb.badge_name = :badge_{idx})"
            )
            params[f"badge_{idx}"] = name
    ...
    # Sort
    sort_clause = _snapshot_sort_clause(sort_by, sort_order)
    ...
    rows = db.execute(
        text(f"SELECT s.* FROM alert_candidate_snapshots s WHERE {where} ORDER BY {sort_clause} LIMIT :limit OFFSET :offset"),
        params,
    ).fetchall()

    items = [_snapshot_row_to_response(dict(r._mapping)) for r in rows]
    items = _refresh_live_relations_for_snapshot_items(db, items)

    # Filter options from snapshot
...
def _build_snapshot_filter_options_v3(db, version, base_where, base_params):
    """Build filter options from snapshot table. Pure SQL aggregation."""
    result = _snapshot_empty_filter_options()
    ...
    # device_tags from live tag tables so header options match rendered rows.
    rows = db.execute(text(
        "SELECT DISTINCT t.name FROM device_tags dt "
        "JOIN tags t ON t.id = dt.tag_id "
        "LEFT JOIN tag_batches tb ON tb.id = t.batch_id "
        "JOIN alert_candidate_snapshots s ON UPPER(s.device_id) = UPPER(dt.device_id) "
        f"WHERE {base_where} "
        "AND (t.batch_id IS NULL OR tb.status IS NULL OR tb.status != 'deleted') "
        "ORDER BY t.name"
    ), base_params).fetchall()
...
def query_alert_candidates(
    ...
    # device_tags comes from frontend as tag names — convert to IDs for SQL filtering
    device_tags = _resolve_tag_filter_ids(db, device_tags)
    exclude_device_tags = _resolve_tag_filter_ids(db, exclude_device_tags)

    try:
        from backend.services.snapshot_builder import get_active_snapshot_version

        active_version = get_active_snapshot_version(db)
        if active_version:
        ...
        import logging
        logging.getLogger(__name__).exception("snapshot candidate query failed; falling back to live query")
    ...
    items, total = _query_candidate_items(
        db,
        where,
        params,
        page,
    ...
    return {
        ...
        "page": page,
        "page_size": page_size,
        "meta": _candidate_scope_meta(snapshot_status="live"),
        "filter_options": filter_options,
    }
