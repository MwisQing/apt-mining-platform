...
def _patch_snapshot_for_device_tags(db, device_ids):
    """Incrementally update snapshot rows affected by device tag changes."""
    try:
        from backend.services.snapshot_builder import patch_snapshot_for_device_tags
        if device_ids:
            patch_snapshot_for_device_tags(db, device_ids)
    except Exception:
        import logging
        logging.getLogger(__name__).exception("patch_snapshot_for_device_tags failed for device_ids=%s", device_ids)


def _patch_snapshot_for_tag_color(db, tag_id):
    """Update snapshot tag subtable when tag color changes."""
    try:
        from backend.services.snapshot_builder import patch_snapshot_for_tag_color
        patch_snapshot_for_tag_color(db, tag_id)
    except Exception:
        import logging
        logging.getLogger(__name__).exception("patch_snapshot_for_tag_color failed for tag_id=%s", tag_id)
...
def update_tag(tag_id: int, data: Any = Body(...), db=Depends(get_db)):
    ...
    if updates:
        ...
        _patch_snapshot_for_tag_color(db, tag_id)
        db.commit()
    return {"ok": True}
...
def create_batch(data: Any = Body(...), db=Depends(get_db)):
    ...
    _invalidate_candidate_cache()
    _patch_snapshot_for_device_tags(db, _clean_device_ids(devices))
    db.commit()
    return result
...
async def import_text_files(files: list[UploadFile] = File(...), db=Depends(get_db)):
    ...
    for file in files:
        ...
        affected_devices.extend(device_ids)

    _invalidate_candidate_cache()
    _patch_snapshot_for_device_tags(db, _clean_device_ids(affected_devices))
    db.commit()
    return {
        "imported": imported,
        "skipped": skipped,
...
def delete_batch(batch_id: int, db=Depends(get_db)):
    ...
    if not batch:
        raise HTTPException(status_code=404, detail="Batch not found")
    # Get affected device IDs before soft-delete for snapshot patch
    affected_devices = _resolve_batch_device_ids(db, batch_id, batch)
    ...
    _invalidate_candidate_cache()
    _patch_snapshot_for_device_tags(db, affected_devices)
    db.commit()
    return {"ok": True}
...
def remove_devices_from_batch(batch_id: int, data: Any = Body(...), db=Depends(get_db)):
    ...
    _invalidate_candidate_cache()
    _patch_snapshot_for_device_tags(db, devices)
    db.commit()
...
def add_device_tag(data: Any = Body(...), db=Depends(get_db)):
    ...
    _invalidate_candidate_cache()
    _patch_snapshot_for_device_tags(db, [device_id])
    db.commit()
