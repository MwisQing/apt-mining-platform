...
def _patch_snapshot_for_trace(db, target, port):
    """Incrementally update snapshot rows affected by IOC note change."""
    try:
        from backend.services.snapshot_builder import patch_snapshot_for_trace
        patch_snapshot_for_trace(db, target, port or "")
    except Exception:
        import logging
        logging.getLogger(__name__).exception("patch_snapshot_for_trace failed for target=%s port=%s", target, port)
...
def create_traced(data: Any = Body(...), db=Depends(get_db)):
    ...
    for item in items:
        ...
        if cursor.lastrowid:
            ...
            changed_pairs.append((target, port or ""))
    if ids:
        write_audit(db, "create_traced", "traced_target", ",".join(str(i) for i in ids), {"count": len(ids)})
    _invalidate_candidate_cache()
    for target, port in changed_pairs:
        _patch_snapshot_for_trace(db, target, port)
    db.commit()
...
async def import_traced(file: UploadFile = File(...), db=Depends(get_db)):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if result.rowcount:
                ...
                changed_pairs.add((target, port or ""))
            else:
                skipped += 1
    ...
    if inserted or skipped:
        _invalidate_candidate_cache()
    for target, port in sorted(changed_pairs):
        _patch_snapshot_for_trace(db, target, port)
    db.commit()
...
def update_traced(traced_id: int, data: Any = Body(...), db=Depends(get_db)):
    ...
    if updates:
        old_target = current_map.get("target")
        old_port = current_map.get("port") or ""
        try:
            ...
            _invalidate_candidate_cache()
            _patch_snapshot_for_trace(db, old_target, old_port)
            _patch_snapshot_for_trace(db, normalized["target"], normalized["port"] or "")
            db.commit()
...
def delete_traced(traced_id: int, db=Depends(get_db)):
    # Save target/port before deletion for snapshot patch
    current = db.execute(text(
        "SELECT target, COALESCE(port, '') FROM traced_targets WHERE id = :id"
    ), {"id": traced_id}).fetchone()
    target = current[0] if current else None
    port = current[1] if current else ""
