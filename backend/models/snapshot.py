...
class AlertCandidateSnapshot(Base):
    ...
    # Precompute v3: pre-stored JSON to avoid JOIN on subtables
    badges_json = Column(Text, default="[]")
    device_tags_json = Column(Text, default="[]")
    device_event_json = Column(Text)

    updated_at = Column(Text, nullable=False)

    __table_args__ = (
        UniqueConstraint("snapshot_version", "alert_id"),
    )
...
# Precompute v3: indexes for incremental updates and fast queries
Index("idx_snap_target_port", AlertCandidateSnapshot.snapshot_version, AlertCandidateSnapshot.target, AlertCandidateSnapshot.port)
Index("idx_snap_device_id", AlertCandidateSnapshot.snapshot_version, AlertCandidateSnapshot.device_id)
Index("idx_snap_date_range", AlertCandidateSnapshot.snapshot_version, AlertCandidateSnapshot.first_alert_time, AlertCandidateSnapshot.last_alert_time)
