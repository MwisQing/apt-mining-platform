...
func (r *EventRepo) CreateEventTx(name, color, note string, devices []string, iocs []IOC) (int, error) {
	...
	for _, ioc := range iocs {
		if _, err := tx.Exec(
			...
			id, strings.ToLower(ioc.Target), ioc.Port,
		); err != nil {
			tx.Rollback()
			return 0, err
		}
	...
	// Create initial followup record
	if _, err := tx.Exec(
		"INSERT INTO event_followups (event_id, action_type, created_at, note) VALUES ($1, 'note', $2, $3)",
		id, time.Now(), "创建事件: "+name,
	); err != nil {
		tx.Rollback()
		return 0, err
	}

	// Auto-tag devices with "事件挖掘: {event_name}" (is_permanent=1)
	if len(devices) > 0 {
		tagName := "事件挖掘: " + name
		var tagID int64
		err = tx.QueryRow(
			"SELECT id FROM tags WHERE name = $1 AND is_permanent = 1",
			tagName,
		).Scan(&tagID)
		if err == sql.ErrNoRows {
			err = tx.QueryRow(
				"INSERT INTO tags (name, color, is_permanent, batch_id, created_at) VALUES ($1, $2, 1, NULL, $3) RETURNING id",
		...
		for _, d := range devices {
			...
			if _, err := tx.Exec(
				"INSERT INTO device_tags (device_id, tag_id, created_at) VALUES (UPPER($1), $2, $3) ON CONFLICT DO NOTHING",
				d, tagID, time.Now(),
			); err != nil {
				tx.Rollback()
				return 0, err
...
func (r *EventRepo) UpdateEvent(id int, name, color, status, note string) error {
	...
	if name != "" {
		updates = append(updates, fmt.Sprintf("event_name = $%d", argIdx))
	...
	args = append(args, id)
	sql := fmt.Sprintf("UPDATE mined_events SET %s WHERE id = $%d", strings.Join(updates, ", "), argIdx)
	if _, err := r.DB.Exec(sql, args...); err != nil {
		return err
	}

	// Record status change followup
	if status != "" {
		r.AddFollowup(id, "status_change", "状态变更为: "+status)
	}
	return nil
}
...
func (r *EventRepo) AddIOCs(eventID int, iocs []IOC) error {
	for _, ioc := range iocs {
		if _, err := r.DB.Exec(
			"INSERT INTO mined_event_iocs (event_id, target, port) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING",
			eventID, strings.ToLower(ioc.Target), ioc.Port,
		); err != nil {
			return err
		}
	}
