...
func BuildWhereSQL(p *CandidateQueryParams) (string, []interface{}, int) {
	...
	if p.Keyword != "" {
		...
		addWhere(fmt.Sprintf(
			"to_tsvector('simple', COALESCE(a.device_id,'') || ' ' || COALESCE(a.source_ip,'') || ' ' || COALESCE(a.target,'') || ' ' || COALESCE(a.threat_type,'') || ' ' || COALESCE(a.std_apt_org,'') || ' ' || COALESCE(a.apt_org,'')) @@ plainto_tsquery('simple', $%d)",
			argIdx,
		), keyword)
		argIdx++
	}
...
traced AS (
    SELECT target, COALESCE(port, '') AS port, 'active' AS trace_status, note AS trace_note FROM traced_targets
),
device_tag_names AS (
    SELECT dt.device_id,
...
scored AS (
    ...
    LEFT JOIN events e ON e.target = a.target AND e.port = COALESCE(a.port, '')
    LEFT JOIN traced tr ON tr.target = a.target AND tr.port = COALESCE(a.port, '')
