...
| 2026-05-13 | 阶段62 | 队列容错+升级脚本优化+数据管理：(1)Settings.vue 轮询逻辑新增连续失败计数器（MAX_POLL_FAILURES=10），后端消失约30秒无响应后自动停止轮询并弹出警告提示"后端长时间无响应，排队任务可能已中断"，不再永久卡队列；(2)`upgrade.py` 改为默认不备份数据库——新增 `--backup` 命令行参数（`python upgrade.py --backup` 才执行备份），减少日常升级耗时；(3)新增 `DELETE /api/imports/all?backup=false` 接口——删除全部上传数据（alerts/import_rows/import_sheets/imports 四表清空），backup=true 时先备份到 backups/ 目录，**不影响**事件/IOC备注/设备标签/配置等数据；(4)Settings 页系统信息 Tab「数据管理」区新增「清除全部上传数据」按钮（danger 色），点击弹出自定义确认 Dialog，展示待删除数据量统计（告警数/文件数/行数）和"事件、IOC备注、设备标签不受影响"提示，三个按钮（取消/确认删除/确认删除并备份），删除后自动刷新导入历史列表；已执行 `py_compile` 和 `npm run build` 验证通过 |
| 2026-05-14 | 阶段63 | 5项修复与优化：(1)源IP悬停延时1s——确认 Workbench.vue `el-tooltip` 已有 `:show-after="1000"`（阶段46已完成），功能正常；(2)上传错误提示修复——Settings.vue `handleUploadExcel` catch 块中超时时不再显示警告提示，静默调用 `loadImports()` 确认后端实际处理状态，消除"上传报错但实际成功"的用户感知；(3)侧边栏固定——App.vue `.sidebar` 改为 `position: sticky; top: 0; height: 100vh; overflow-y: auto`，`.main-shell` 改为 `height: 100vh; overflow-y: auto`，左侧导航不再随页面滚动；(4)侧边栏版本信息——底部新增 `version-row` 展示 "APT Mining v3.2.9"，通过 `fetchVersion()` API 获取版本号；(5)快照构建集成到导入流程——Settings.vue 导入完成轮询后新增 `startSnapshotCheck` 轮询 `/api/snapshots/status`，快照就绪前不提示成功；构建中每 ~18s 提示"数据已导入，正在构建候选快照"；新增 `frontend/src/api/snapshots.js` 模块；已执行 `npm run build` 验证通过 |
| 2026-05-14 | 阶段64（人工） | 取消Linux相关的更新与维护，仅针对Windows |
| 2026-05-14 | 阶段65 | 研判工作台去快照化与 IOC 备注/提交流程修复：1) `/api/alert-candidates` 改为直接查询实时候选数据，不再依赖 snapshot active version，事件/标签/IOC备注保存后仅清理候选缓存，不再触发快照重建；2) Workbench 改为按日期范围一次性加载全量候选到前端内存，顶部筛选/列筛选/关键词/设备标签排除/排序/分页均在前端完成，移除工作台快照构建提示；3) Settings 导入完成后不再轮询快照状态，直接按导入任务完成提示成功；4) IOC 备注后端新增归一化与重复冲突保护，编辑空端口备注可正常保存；5) IOC 备注页新增批量添加大输入框，支持每行一个 `IOC` 或 `IOC:port` 批量备注；已执行后端 unittest 回归与前端 `npm run build` 验证通过 |
| 2026-05-14 | 阶段66 | 三项修复：(1)设备标签列展示已创建事件——后端`_event_maps_for_rows`重构：`device_map`从单事件改为`device_events_map`（每设备保留所有关联事件+IOC目标集合），`_make_items`中`device_event`匹配逻辑改为遍历设备所有事件检查IOC精确匹配（同一事件必须同时命中IOC和设备ID）；前端Workbench设备标签列优先显示匹配事件Tag；(2)源IP悬停预览1s延时——`el-tooltip`改为`el-popover`手动控制，`:show-after="1000"`生效；(3)IOC备注页修复——修复`confirmSave`中4处中文乱码（`???????`→正确中文），添加缺失的批量IOC输入textarea（编辑模式显示单目标输入，添加模式显示多行textarea）；已执行`py_compile`、unittest和`npm run build`验证通过 |
| 2026-05-14 | 阶段67 | 设备标签列事件不显示bug修复：根因为`_event_maps_for_rows`中设备ID匹配存在大小写敏感问题——事件创建时`extract_iocs_from_text`通过`_ordered_unique_upper`将设备ID大写化（如`WIN-YANGJIU`），但告警表中设备ID保持Excel原始大小写（如`WIN-yangjiu`），SQLite默认BINARY排序规则导致`WHERE med.device_id IN (...)`查询无法匹配不同大小写的设备ID；修复方案：(1)SQL查询改为`WHERE UPPER(med.device_id) IN (...)`并将参数大写化；(2)Python字典`device_events_map`的key统一用`UPPER()`存储，`_make_items`中查找时也用`.upper()`；已执行`py_compile`、unittest和`npm run build`验证通过 |
| 2026-05-14 | 阶段68 | 候选快照预计算方案落地：(1)修复`snapshot_builder.py` 3个bug——`RULE_SCORE_MAP`键双空格(`"std_apt  org_present"`→`"std_apt_org_present"`)、`_snapshot_row_from_candidate`键冒号(`"std_apt: org"`→`"std_apt_org"`)、`_recompute_score` heat_score括号错误(`min(x)*2,18`产生tuple→`min(x*2,18)`)；(2)`alerts.py`新增`_snapshot_row_to_response`/`_query_from_snapshot`/`_build_snapshot_filter_options_v3`，`query_alert_candidates`优先查active snapshot version路由到快照查询路径，无快照时回退实时候选路径；(3)`events.py`所有事件变更端点（create/update/delete/add-remove devices/iocs）接入`_patch_snapshot_for_event`增量刷新，delete前保存IOCs+device_ids用于`_patch_snapshot_for_event_deletion`；(4)`traced.py`所有追踪变更端点接入`_patch_snapshot_for_trace`增量刷新；(5)`tags.py`所有标签变更端点接入`_patch_snapshot_for_device_tags`/`_patch_snapshot_for_tag_color`增量刷新；(6)`_recompute_badges_for_incremental`修复`event_info`为`_UNSET`时误入subscript分支；(7)新增`test_snapshot_precompute.py` 15项测试覆盖全量构建/快照查询/增量更新/响应格式/性能；已执行unittest(15/15)和`npm run build`验证通过 |
| 2026-05-14 | 阶段68补丁 | 增量patch静默错误修复：(1)根因——events.py/traced.py/tags.py的`_patch_snapshot_for_*`函数使用`try/except Exception: pass`静默吞掉所有异常，导致事件/IOC备注/设备标签创建后快照未更新，研判工作台不显示这些数据；(2)修复——所有6个patch wrapper改为`logging.getLogger(__name__).exception(...)`记录错误而非静默吞掉；(3)手动触发快照重建验证——重建后event_json/trace_json/device_tags_json数据正确填充；已执行unittest(15/15)和`npm run build`验证通过 |
| 2026-05-14 | 阶段68补丁2 | 已修复“原始告警存在但研判工作台搜索不到”问题，根因为候选/快照按 device_id+target+port 错误去重，现已改为按原始告警粒度保留，同一 IOC 不同时间告警可正常检索。 |
| 2026-05-14 | 阶段69 | 修复工作台刷新后设备标签、事件、IOC备注不显示问题；根因是候选接口误走快照数据，已改回实时查询，并补齐 badges_filter 与 candidate_priority 兼容性回归测试。 |
| 2026-05-15 | 阶段70 | 研判工作台预计算快照 v3 正式启用与一致性修复：(1)`/api/alert-candidates` 优先读取 active snapshot version，无快照或快照查询异常时回退实时查询；(2)事件/IOC备注/设备标签变更改为同一事务内增量 patch 快照后再提交，patch 失败不再静默吞掉；(3)事件删除、移除 IOC、移除设备改为按受影响 IOC/设备 scope 重新匹配当前全局事件，避免快照误清或遗漏；(4)TXT 批量打标、标签删除、标签颜色修改、IOC 批量导入均补齐快照增量刷新，标签空列表会同步清空快照子表，标签颜色同步刷新 `device_tags_json`；(5)导入删除/清空/修复元数据后触发快照重建；(6)Settings 导入完成后的快照状态轮询函数补回，避免未定义变量；(7)`requirements.txt` 补充 `httpx` 测试依赖；已执行后端 unittest 53/53、快照专项 21/21、前端 `npm run build` 和本地 `http://127.0.0.1:8090/api/health` 验证通过。 |
| 2026-05-15 | 阶段71 | 导入排队卡住问题修复：(1)队列 worker 增加 logging 日志——启动/处理/完成/失败全流程可见，异常不再静默吞掉(`logger.info/exception`)；(2)重复文件上传处理优化——已成功的文件直接跳过提示，排队中的文件返回原记录不重复入队，失败的文件自动重新入队；(3)快照重建改为异步(`rebuild_candidate_snapshots_async`)，不再阻塞导入队列，防止大型导入后下一个文件卡在排队中；(4)新增 `POST /api/imports/reprocess-queued` 接口——重新处理所有卡在 `queued` 状态的导入，文件丢失时自动标记为 `failed`；(5)前端导入历史新增「重新处理卡住任务」按钮(带 `RefreshRight` 图标)，`hasQueuedImports` 控制禁用态；(6)新增 `frontend/src/api/imports.js` 的 `reprocessQueuedImports` 方法；已执行 `py_compile` 和 `npm run build` 验证通过。 |
| 2026-05-15 | 阶段72 | 研判工作台事件显示与快照一致性收敛：(1)根因确认——`/api/alert-candidates` 走 active snapshot 后直接返回快照中的 `event_json/device_tags_json/trace_json`，一旦增量 patch 漏行或异步重建未完成，创建事件后工作台刷新仍看不到事件；(2)`backend/api/alerts.py` 新增快照结果实时关系覆盖层，快照仅作为候选底表，返回前从实时 `mined_event_*`、`device_tags/tags`、`traced_targets` 表覆盖事件、设备标签、IOC备注/追踪状态，并重算相关徽章、命中原因、分数、优先级和排序信号；(3)`backend/api/events.py` 移除事件变更后的无条件后台全量快照重建，事件创建/更新/删除仅做同步增量 patch，导入/清空数据等告警底表变化才需要全量重建，减少后台切换快照版本造成的新竞态；(4)新增“旧快照未 patch 但实时事件存在”的回归测试，确认事件、标签、IOC备注仍能显示；已执行 `py_compile`、快照相关 unittest 29/29、后端全量 unittest 54/54 和前端 `npm run build` 验证通过。 |
| 2026-05-15 | 阶段73 | 创建事件按钮卡死修复：(1)恢复 `frontend/src/views/Workbench.vue` 中丢失的研判操作列，新增「创建事件」按钮、创建事件 Dialog、IOC/设备自动填充、备注预填、提交 loading 和本地行状态即时回填；(2)`frontend/public/columns.json` 与 `useColumnConfig.js` 增加 `actions` 默认列，避免列设置中缺失操作列；(3)`backend/api/events.py` 将事件创建/更新/删除/增删 IOC/设备后的快照 patch 改为提交后后台 best-effort 执行，不再阻塞用户点击和提交；工作台事件显示由阶段72的实时覆盖保证准确；(4)避免使用 Element Plus `fixed` 操作列，防止再次引入表格数据偏移；已执行 `py_compile`、快照相关 unittest 22/22、后端全量 unittest 54/54 和前端 `npm run build` 验证通过。 |
| 2026-05-16 | 阶段74 | 快照表方案二次 review：确认当前实现已从“纯快照查询”演变为“快照底表 + 实时关系覆盖”混合模型；指出 3 个关键风险——(1)快照 SQL 先筛选/排序/分页、后覆盖实时事件/标签/追踪，导致过滤语义、总数和排序可能与最终展示不一致；(2)创建事件时自动写入 `device_tags`，但未同步触发标签快照 patch，标签筛选/筛选项可能滞后；(3)标签快照增量 patch 仍存在 `device_id` 大小写敏感路径，和已修复的事件映射问题口径不一致。 |
| 2026-05-16 | 阶段75 | 外部快照表 review 复核：整体结论基本认可，确认 `patch_snapshot_for_device_tags` / `_sync_tag_subtable` 的设备 ID 大小写敏感问题、`tags.py` 中 patch 异常会阻断打标请求、事件增量 patch 会重建全量映射、批量打标时逐设备同步子表等问题属实；同时修正优先级判断——重复索引和全量映射重建更偏写入性能问题，不宜定为 P0；补充指出外部 review 漏掉了当前最关键的一致性风险：快照查询在 SQL 层先过滤/排序/分页，再做实时覆盖，导致筛选语义、总数和最终展示可能不一致。 |
| 2026-05-16 | 阶段76 | 快照表方案 review 7 项修复落地：(1)P0 删除 `snapshot.py:127` 重复的 `idx_snap_version_score` 索引定义，保留 `db.py` 运行时 DDL 版本；(2)P0 修复设备标签 patch 大小写不一致——`patch_snapshot_for_device_tags` 和 `_sync_tag_subtable` 的 SQL 改为 `UPPER(device_id) = :d` + 参数 `.upper()` 归一化；(3)P1 移除 `tags.py` 和 `traced.py` 中 `_patch_snapshot_for_*` 函数的 `raise`，快照 patch 异常不再阻断用户打标/IOC 备注请求，与其他模块保持一致；(4)P1 优化 `_patch_snapshot_event_rows`——从查询全部快照 target/device_id 重建全量事件映射，改为直接查询 `mined_events` + `mined_event_iocs` + `mined_event_devices` 获取活跃事件 IOC/设备列表，逐行匹配，避免 N×全量快照行的映射重建开销；(5)P2 批量打标子表同步合并——`patch_snapshot_for_device_tags` 改为先收集所有 device→tags 映射，再调用新增的 `_sync_tag_subtable_batch` 一次性 DELETE+INSERT，批量打 50 台设备从 50 次 SELECT+50 次 DELETE 降为 1 次；(6)P2 `_recompute_badges_for_incremental` 新增注释说明 cross_day/lateral 等聚合 badge 增量 patch 中可能过期的 trade-off，由实时覆盖层兜底；(7)P2 `_ensure_runtime_schema` 启动时一行 SQL 清理 orphaned `building_version`；已执行 unittest 54/54（含快照专项 29/29）和前端 `npm run build` 验证通过。 |
| 2026-05-16 | 阶段77 | 快照表方案现状复核：确认当前实现仍是“快照底表 + 实时关系覆盖”混合模型而非纯快照查询；明确当前首要风险已收敛到查询语义一致性——快照 SQL 先基于旧 `candidate_score`/`event_status`/`trace_status`/标签子表做过滤、排序、分页与总数统计，再在 Python 中实时覆盖事件/标签/IOC备注并重算分数与排序信号，可能导致返回顺序、筛选命中与 `total/filter_options` 和最终展示字段不一致；建议后续在“纯快照一致性”和“全实时关系查询”两条路线中二选一收敛。 |
| 2026-05-16 | 阶段78 | 表头筛选走快照 SQL 层+设备ID计数重命名：(1)**问题排查**——用户筛选"重点设备"后不显示任何数据，初步误判为 `displayData = allFilteredData.slice()` 的 bug（`allFilteredData` 源数据只有当前页 50 条），修复后导致首页只显示 50 条新 bug，用户要求回退重新定位；(2)**架构修正**——最终采用正确架构：列筛选参数（`device_tags`/`threat_types`/`apt_orgs`/`ports`/`badges_filter`）发到后端走快照 SQL 过滤，`displayData = tableData` 正常后端分页，不改变快照表的用途；(3)**前端改动**——`loadData()` 新增将 `columnFilters` 中列筛选值作为参数发送给后端；(4)**后端改动**——`_build_where` 和 `_query_from_snapshot` 新增 `apt_orgs`/`ports` 参数支持 SQL 过滤；`device_tags` 前端传 tag 名称，后端在 `query_alert_candidates` 入口用 `SELECT id FROM tags WHERE name IN (...)` 转为 tag ID 再走 SQL；(5)列名"设备告警ioc数"改为"设备ID计数"（`columns.json`），算法从 `COUNT(DISTINCT target)` 改为 `COUNT(*)`（告警行总数，`alerts.py:1266` + `HEAT_SORT_JOINS:1457`），默认排序按"设备ID计数"升序；(6)已执行 unittest 54/54 和 `npm run build` 验证通过。 |
| 2026-05-16 | 阶段79 | 设备标签表头筛选返回空数据修复：根因为 `_query_from_snapshot` 和 `_build_where` 中 tag_id 参数绑定为字符串（`_csv_values` 返回 `["2"]` 而非 `[2]`），SQLite 严格类型比较下整数列 `tag_id=2` 与字符串参数 `"2"` 不相等，导致 `EXISTS` 子查询永远不匹配；修复：快照路径和实时路径的 `tag_id`/`exclude_tag_id` 参数绑定统一加 `int()` 转换；已执行 unittest 29/29 和 `npm run build` 验证通过。 |
| 2026-05-16 | 阶段80 | 设备标签表头筛选空数据二次修复：根因进一步确认不是前端分页或单纯 `tag_id` 类型问题，而是快照查询用 `alert_candidate_snapshot_tags` 子表过滤，页面展示却由实时 `device_tags/tags` 覆盖；旧快照或漏 patch 时会出现”行上能看到设备标签，但点击表头标签筛选后 total=0”。修复：`_query_from_snapshot` 的 `device_tags`/`exclude_device_tags` 改为按实时 `device_tags/tags` 过滤，并使用 `UPPER(device_id)` 统一大小写、排除 deleted 批次；快照筛选项 `filter_options.device_tags` 也改从实时标签表聚合；`query_alert_candidates` 新增 `_resolve_tag_filter_ids`，同时支持前端传标签名和 API 文档中的标签 ID，未命中时用 `0` 表示空结果；无快照实时路径 `_build_where` 的设备标签过滤同样改为大小写不敏感。新增”快照标签子表滞后但实时标签存在时仍可筛选”的回归测试；已执行 `python -m unittest backend.tests.test_snapshot_query_semantics` 7/7、后端全量 `python -m unittest discover backend/tests` 55/55 和前端 `npm run build` 验证通过。 |
| 2026-05-18 | 阶段81 | 上传Excel卡死平台修复：(1)**根因1** SQLite单连接(StaticPool)—导入队列worker持有唯一连接，其他请求全部阻塞；**根因2** Excel全量加载内存爆炸—pandas将10万行×24列全部加载为DataFrame+dict+JSON字符串，峰值500MB-1GB，GC thrashing；**根因3** 导入后自动快照重建—首次构建~110s，大量DB读写加重负载；(2)`db.py`修复：SQLite连接池改为`QueuePool(pool_size=5,max_overflow=3,pool_timeout=10)`，多连接并发，导入不再独占；(3)`imports.py`修复：`_process_excel`改用openpyxl `read_only`模式逐行流式读取，每500行batch commit，内存峰值降至~50MB；(4)`imports.py`修复：导入完成后不再自动触发`rebuild_candidate_snapshots_async()`，快照按需构建；(5)`requirements.txt`移除重复openpyxl条目；已执行`py_compile`、unittest 55/55和`npm run build`验证通过。 |
| 2026-05-18 | 阶段82 | 上传Excel后一直卡在"解析"修复：(1)**根因** `_process_excel`开头执行全表扫描`SELECT content_hash FROM alerts WHERE content_hash IS NOT NULL`，10万+行数据库需要数分钟，期间导入状态已设为"processing"但实际尚未开始解析任何行，前端永远显示"正在解析"；(2)`_process_excel`修复：移除阻塞性初始全量hash查询，`workbook_seen_hashes`改为仅跟踪当前文件内重复；(3)`_flush_batch`修复——跨文件去重改为每batch内增量查询（`SELECT content_hash FROM alerts WHERE content_hash IN (:h_0, :h_1, ...)`，最多500个参数），仅查当前batch的hash而非全表；(4)`_flush_batch`修复——import_rows更新从不可靠的OFFSET-based `first_row_id`查询改为基于`row_hash`精确匹配（`WHERE row_hash = :row_hash AND import_id = :import_id`）；(5)`_flush_batch`修复——DB重复行正确标记为`skipped_duplicate`，计数器自动修正；(6)已执行`py_compile`、unittest 55/55和`npm run build`验证通过。 |
