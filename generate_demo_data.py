...
- C2 beacon patterns: same device→same target→same port across multiple days
...
- All metadata fields populated (intel_position, disposal_action, analysis_status, is_focused)
"""
import pandas as pd
import random
import os
...
# ── Metadata pools (now with full variation) ──
INTEL_POSITIONS = ["设备侧"] * 40 + ["目标侧"] * 25 + ["链路侧"] * 20 + ["情报源"] * 10 + [""] * 5
DISPOSAL_ACTIONS = ["远程"] * 35 + ["封堵"] * 20 + ["隔离"] * 15 + ["观察"] * 15 + ["通报"] * 10 + [""] * 5
ANALYSIS_STATUSES = [""] * 60 + ["研判中"] * 20 + ["已研判"] * 15 + ["忽略"] * 5
...
# Cache: IP → device lookup for lateral movement layer
ip_to_device = {ip: dev for dev, ip in device_source_ips.items()}
...
def make_row(device_id, first_time, last_time, source_ip, target, target_type, port,
             ...
             intel_position, disposal_action, dns_ip, down_traffic, up_traffic, asset_type,
    ...
    for _ in range(num_devices):
        ...
        for day_offset in range(0, BEACON_DAYS, random.randint(1, 3)):
            ...
            rows.append(make_row(
                ...
                random.choice(INTEL_POSITIONS), random.choice(DISPOSAL_ACTIONS),
...
for src_ip in lateral_source_ips:
    ...
    if num_targets <= 0:
        break
    # Find device for this IP via cache (O(1) lookup)
    dev = ip_to_device.get(src_ip)
    if not dev:
        continue
    ...
    for _ in range(num_targets):
        ...
        rows.append(make_row(
            ...
            random.choice(INTEL_POSITIONS), "隔离",
...
for dev in SCAN_DEVICES:
    ...
    for _ in range(random.randint(5, 25)):
        ...
        rows.append(make_row(
            ...
            random.choice(INTEL_POSITIONS), "观察",
            "", 0, 0, random.choice(ASSET_TYPES),
            random.choice(ANALYSIS_STATUSES), "否",
        ))
...
for i in range(target_count):
    ...
    intel_tags = random.choice(["apt,c2", "c2", "远控", ""]) if "APT" in threat_type or "远控" in threat_type else ""
...
unique_devices = len(set(r["设备ID"] for r in rows))
unique_targets = len(set(r["外联目标"] for r in rows))
non_empty_intel_pos = sum(1 for r in rows if r.get("情报位置"))
non_empty_disposal = sum(1 for r in rows if r.get("处置动作"))
non_empty_status = sum(1 for r in rows if r.get("研判状态"))
non_empty_focused = sum(1 for r in rows if r["重点关注"] == "是")

print(f"APT/C2-related rows: {apt_rows:,}")
print(f"Unique devices: {unique_devices:,}")
print(f"Unique targets: {unique_targets:,}")
print(f"情报位置 populated: {non_empty_intel_pos:,} ({non_empty_intel_pos/len(rows)*100:.0f}%)")
print(f"处置动作 populated: {non_empty_disposal:,} ({non_empty_disposal/len(rows)*100:.0f}%)")
print(f"研判状态 populated: {non_empty_status:,} ({non_empty_status/len(rows)*100:.0f}%)")
print(f"重点关注=是: {non_empty_focused:,} ({non_empty_focused/len(rows)*100:.0f}%)")
