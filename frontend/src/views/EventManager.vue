        ...
        <template v-else>
          <div class="detail-header">
            <div class="detail-title-row">
              <div class="detail-color-dot" :style="{ backgroundColor: selectedEvent.color || '#409eff' }"></div>
              <el-input
                v-model="editName"
                size="large"
                class="title-input"
              ...
              <div class="detail-section">
                ...
                <div class="color-presets">
                  <span
                    ...
                    :class="{ 'color-preset--active': selectedEvent.color === c }"
                    :style="{ backgroundColor: c }"
                    @click="handleChangeColor(c)"
                  ></span>
                  <el-color-picker v-model="editColor" size="small" class="color-custom-picker" @change="handleSaveColor" />
                </div>
              </div>
...
async function handleSaveName() {
  if (!selectedEvent.value || editName.value === selectedEvent.value.event_name) return
  try {
    await updateEvent(selectedEvent.value.id, { event_name: editName.value })
    selectedEvent.value.event_name = editName.value
    loadEvents()
  } catch (e) {
    ElMessage.error(`保存名称失败: ${e.message}`)
  }
}
...
async function handleSaveColor() {
  ...
  try {
    await updateEvent(selectedEvent.value.id, { color: editColor.value })
    selectedEvent.value.color = editColor.value
    loadEvents()
  } catch (e) {
    /* silent */
  }
...
function handleSaveNote() {
  ...
  noteSaveTimer = setTimeout(async () => {
    if (!selectedEvent.value) return
    try {
      await updateEvent(selectedEvent.value.id, { note: editNote.value })
    } catch (e) {
      /* silent */
    }
  }, 500)
...
async function handleExtractIocs() {
  ...
  try {
    ...
    if (result.iocs?.length) {
      ...
      handleSaveIocs()
      ElMessage.success(`识别到 ${result.iocs.length} 个 IOC`)
    } else {
...
async function handleExtractDevices() {
  ...
  try {
    ...
    if (result.devices?.length) {
      ...
      handleSaveDevices()
      ElMessage.success(`识别到 ${result.devices.length} 个设备 ID`)
    } else {
...
async function handleSaveDevices() {
  if (!selectedEvent.value || savingDevices.value) return
  ...
  try {
    ...
    for (const d of currentDevices) {
      if (!newDevices.some((nd) => nd.toUpperCase() === String(d).toUpperCase())) {
        try {
          await removeDevice(selectedEvent.value.id, d)
        } catch (e) {
          /* silent */
        }
      }
    ...
    const toAdd = newDevices.filter((nd) => !currentSet.has(nd.toUpperCase()))
    if (toAdd.length > 0) {
      await addDevices(selectedEvent.value.id, { devices: toAdd })
      ElMessage.success(`已添加 ${toAdd.length} 个设备`)
    }
    selectedEvent.value.devices = newDevices
  } catch (e) {
    ElMessage.error(`保存设备失败: ${e.response?.data?.detail || e.message}`)
  } finally {
    savingDevices.value = false
  }
}

async function handleSaveIocs() {
  if (!selectedEvent.value || savingIocs.value) return
  savingIocs.value = true
  try {
    const lines = editIocs.value.split(/[\n\r]+/).map((s) => s.trim()).filter(Boolean)
    const newIocs = []
    ...
    for (const ioc of currentIocs) {
      ...
      if (!newIocs.some((ni) => `${ni.target}:${ni.port || ''}` === key)) {
        try {
          await removeIoc(selectedEvent.value.id, ioc.target, ioc.port || '')
        } catch (e) {
          /* silent */
        }
      }
    ...
    const toAdd = newIocs.filter((ni) => !currentKeys.has(`${ni.target}:${ni.port || ''}`))
    if (toAdd.length > 0) {
      await addIocs(selectedEvent.value.id, { iocs: toAdd })
    }
    selectedEvent.value.iocs = newIocs
  } catch (e) {
    ElMessage.error(`保存 IOC 失败: ${e.message}`)
  } finally {
    savingIocs.value = false
  }
}
...
.page-banner {
  ...
  border-radius: 24px;
  background:
    linear-gradient(135deg, rgba(255, 125, 95, 0.12), transparent 60%),
    var(--panel-strong);
