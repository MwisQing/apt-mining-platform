      ...
      <el-tab-pane label="数据导入" name="import">
        <div class="tab-content">
          ...
          <el-card class="section-card">
            ...
            <!-- 快照构建状态 -->
            <div v-if="snapshotBuildStatus" class="snapshot-status">
              <template v-if="snapshotBuildStatus.status === 'building'">
                <el-icon class="is-loading"><Loading /></el-icon>
                <span>候选快照构建中...</span>
                <span v-if="snapshotBuildStatus.last_error" class="snapshot-status__error">
                  （上次构建异常：{{ snapshotBuildStatus.last_error }}）
                </span>
                <el-tag type="warning" size="small" class="snapshot-status__tag">构建中</el-tag>
              </template>
              <template v-else-if="snapshotBuildStatus.status === 'ready'">
                <el-icon><CircleCheckFilled /></el-icon>
                <span>候选快照已就绪</span>
                <span class="snapshot-status__detail" v-if="snapshotBuildStatus.last_built_at">
                  · {{ formatSnapshotTime(snapshotBuildStatus.last_built_at) }}
                </span>
                <span v-if="snapshotBuildStatus.last_row_count != null" class="snapshot-status__detail">
                  · {{ snapshotBuildStatus.last_row_count }} 条记录
                </span>
                <el-tag type="success" size="small" class="snapshot-status__tag">就绪</el-tag>
              </template>
              <template v-else-if="snapshotBuildStatus.status === 'error'">
                <el-icon><WarningFilled /></el-icon>
                <span>候选快照构建失败</span>
                <span class="snapshot-status__error" v-if="snapshotBuildStatus.last_error">
                  ：{{ snapshotBuildStatus.last_error }}
                </span>
                <el-tag type="danger" size="small" class="snapshot-status__tag">异常</el-tag>
              </template>
            </div>
          </el-card>
            ...
            <template #header>
              <div class="card-header-row">
                ...
                <div style="display:flex;gap:6px;">
                  <el-button size="small" @click="handleReprocessQueued" :disabled="!hasQueuedImports">
...
import { UploadFilled, Upload, Loading, Plus, Delete, ArrowUp, ArrowDown, QuestionFilled, Refresh, RefreshRight, FolderOpened, WarningFilled, CircleCheckFilled } from '@element-plus/icons-vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import {
  uploadExcel, fetchImports, fetchImport, fetchImportSheets, fetchImportRows, downloadImportRowsCsv, deleteImport, repairImportMetadata, deleteAllImports, reprocessQueuedImports
} from '../api/imports'
import {
...
import { fetchSnapshotStatus } from '../api/snapshots'
...
// Snapshot build status display
const snapshotBuildStatus = ref(null)
...
const hasQueuedImports = computed(() => {
...
async function handleReprocessQueued() {
  try {
    await ElMessageBox.confirm(
      '此操作将重新处理所有卡在"排队中"的导入任务。确认继续？',
      '确认重新处理',
    ...
    if (e !== 'cancel') {
      ElMessage.error('重新处理失败: ' + e.message)
    }
  }
}
  ...
  importPollTimer = setInterval(async () => {
    try {
      ...
      if (detail.status !== 'queued' && detail.status !== 'processing' && detail.status !== 'uploaded') {
        ...
        // 导入完成后，检查快照状态——快照就绪前不提示成功
        if (!snapshotCheckStarted) {
          snapshotCheckStarted = true
          startSnapshotCheck(detail.source_file, (msg) => {
...
function startSnapshotCheck(sourceFile, onReady) {
  stopSnapshotPolling()
  let attempts = 0
  ...
  snapshotPollTimer = setInterval(async () => {
    ...
    try {
      ...
      if (isReady && !isBuilding) {
        stopSnapshotPolling()
        snapshotCheckStarted = false
        onReady?.('导入完成，候选快照已就绪：' + sourceFile)
      } else if (isError) {
        stopSnapshotPolling()
        snapshotCheckStarted = false
        ElMessage.warning('导入完成，但候选快照构建失败，请在系统信息中手动重建。')
      } else if (attempts % 6 === 0) {
        ElMessage.info('数据已导入，正在构建候选快照：' + sourceFile)
      }

      if (attempts >= MAX_ATTEMPTS) {
        stopSnapshotPolling()
        snapshotCheckStarted = false
        ElMessage.warning('导入完成，候选快照仍在构建中，请稍后刷新工作台。')
      }
    } catch {
      if (attempts >= 5) {
        stopSnapshotPolling()
        snapshotCheckStarted = false
        onReady?.('导入完成：' + sourceFile)
      }
    }
  }, 3000)
}
...
.snapshot-status__detail {
  font-size: 12px;
  color: var(--text-muted);
}

.snapshot-status__error {
  font-size: 12px;
  color: #F56C6C;
}

.snapshot-status__tag {
  margin-left: auto;
}
