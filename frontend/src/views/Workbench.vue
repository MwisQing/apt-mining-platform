...
async function loadData() {
  ...
  try {
    ...
    if (targetKind.value && targetKind.value !== 'all') {
      params.target_kind = targetKind.value
    }
    if (hideTraced.value !== undefined) {
      params.hide_traced = hideTraced.value
    }
    if (keyword.value) {
    ...
    if (columnFilters.threat_type.length > 0) {
      params.threat_types = columnFilters.threat_type.join(',')
    }
    if (columnFilters.std_apt_org.length > 0) {
      params.apt_orgs = columnFilters.std_apt_org.join(',')
    }
    if (columnFilters.port.length > 0) {
      params.ports = columnFilters.port.join(',')
    }
    if (columnFilters.badges.length > 0) {
      params.badges_filter = columnFilters.badges.join(',')
    }
