    ...
    >
      <el-form :model="eventForm" label-width="84px" class="event-form">
        <el-form-item label="事件名称">
          <el-input
            :model-value="eventForm.event_name"
            maxlength="80"
            show-word-limit
        ...
        <el-form-item label="设备标签">
          <div class="event-tag-row">
            <el-select
              v-model="eventSelectedTags"
              multiple
              filterable
              clearable
              collapse-tags
              collapse-tags-tooltip
              placeholder="选择标签后自动写入设备ID"
              class="event-tag-select"
              :loading="eventDeviceLoading"
              @change="handleEventTagChange"
            >
              <el-option
                v-for="tag in eventTagOptions"
                :key="tag.name"
                :label="`${tag.name}（${tag.count}）`"
                :value="tag.name"
              />
            </el-select>
            <el-button @click="resetEventToCurrentDevice" :disabled="eventDeviceLoading">仅当前设备</el-button>
          </div>
        </el-form-item>
        <el-form-item label="Text模板">
          <el-input
            :model-value="eventForm.note"
            type="textarea"
            :rows="12"
            resize="vertical"
            @input="handleEventNoteInput"
          />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { fetchCandidates, fetchIocDevices } from '../api/candidates'
...
const eventDeviceLoading = ref(false)
const selectedEventRow = ref(null)
const eventSelectedTags = ref([])
const previousEventSelectedTags = ref([])
const eventInsertedByTag = ref({})
const eventTagOptions = ref([])
const eventTagDeviceMap = ref({})
const DEFAULT_EVENT_COLOR = '#FF5722'
...
function buildTagOptions(items) {
  ...
  for (const item of items || []) {
    for (const tag of item.tags || []) {
      ...
      map[tag.name].count += 1
      map[tag.name].devices.push(item.device_id)
    }
  }

  eventTagDeviceMap.value = Object.fromEntries(
    Object.values(map).map((tag) => [tag.name, tag.devices]),
  )
  eventTagOptions.value = Object.values(map).sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'))
}

async function loadEventDevices(row) {
  eventDeviceLoading.value = true
  try {
    const params = {
      target: row.target || '',
      port: row.port || '',
    ...
    const res = await fetchIocDevices(params)
    const items = res?.items || []
    buildTagOptions(items)
  } catch (e) {
    buildTagOptions([])
    ElMessage.warning(`加载 IOC 设备列表失败: ${e.message}`)
  } finally {
    eventDeviceLoading.value = false
  }
}

async function openEventDialog(row) {
  selectedEventRow.value = row
  const target = row.target || ''
  const port = row.port || ''
  const eventName = defaultEventName()
  const iocText = formatIoc(target, port)
  eventForm.event_name = eventName
  eventForm.color = DEFAULT_EVENT_COLOR
  eventForm.devices = row.device_id ? [row.device_id] : []
  eventForm.iocs = (target && port) ? [{ target, port }] : (target ? [{ target: target, port: '' }] : [])
  eventForm.note = buildEventNote({
    ioc: iocText,
    devices: eventForm.devices,
    eventName,
  })
  eventSelectedTags.value = []
  previousEventSelectedTags.value = []
  eventInsertedByTag.value = {}
  eventTagOptions.value = []
  eventTagDeviceMap.value = {}
  eventDialogVisible.value = true
  await loadEventDevices(row)
}

function handleEventNameInput(value) {
  eventForm.event_name = value
  eventForm.note = replaceEventNameInEventNote(eventForm.note, value)
}

function handleEventNoteInput(value) {
  eventForm.note = value
  eventForm.event_name = parseEventNameFromEventNote(value)
}

function handleEventTagChange(nextTags) {
  const result = updateEventNoteDevicesForTagChange({
    text: eventForm.note,
    selectedTags: nextTags,
    previousSelectedTags: previousEventSelectedTags.value,
    tagDeviceMap: eventTagDeviceMap.value,
    insertedByTag: eventInsertedByTag.value,
  })
  eventForm.note = result.text
  eventInsertedByTag.value = result.insertedByTag
  previousEventSelectedTags.value = [...nextTags]
}

function resetEventToCurrentDevice() {
  const row = selectedEventRow.value
  const eventName = eventForm.event_name || defaultEventName()
  eventForm.note = buildEventNote({
    ioc: formatIoc(row?.target, row?.port),
    devices: row?.device_id ? [row.device_id] : [],
    eventName,
  })
  eventSelectedTags.value = []
  previousEventSelectedTags.value = []
...
function patchMatchingRowsEvent(eventId) {
  const row = selectedEventRow.value
  const target = String(row?.target || '').toLowerCase()
  const port = String(row?.port || '')
  const devices = new Set(eventForm.devices.map((device) => device.toUpperCase()))
  const eventInfo = {
    event_id: eventId,
  ...
  for (const item of allTableData.value) {
    const sameIoc = String(item.target || '').toLowerCase() === target && String(item.port || '') === port
    if (!sameIoc) continue
    item.event = eventInfo
    item.event_status = 'active'
    if (devices.has(String(item.device_id || '').toUpperCase())) {
      item.device_event = eventInfo
    }
  }
...
async function submitEvent() {
  ...
  const devices = parseDevicesFromEventNote(eventForm.note)
  if (!devices.length) {
    ElMessage.warning('请在 Text 模板中保留至少一个设备 ID')
    return
  }
  const noteIoc = parseIocFromEventNote(eventForm.note)
  if (!noteIoc.target) {
    ElMessage.warning('请在 Text 模板中保留线索 IOC')
    return
  }
  eventForm.devices = devices
  eventForm.iocs = [noteIoc]
