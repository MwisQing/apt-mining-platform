<template>
  <div class="ioc-notes-page">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑备注' : '添加备注'" width="500px">
...
const form = ref({ target: '', port: '', note: '', targetsText: '' })

function showAddDialog() {
  editingId.value = null
  form.value = { target: '', port: '', note: '', targetsText: '' }
  dialogVisible.value = true
}

function showEditDialog(row) {
  editingId.value = row.id
  form.value = { target: row.target, port: row.port || '', note: row.note || '', targetsText: '' }
  dialogVisible.value = true
}
...
function parseBatchTargets(text) {
  return text
    ...
    .map((line) => {
      ...
      if (match) {
        return { target: match[1].trim(), port: match[2] }
      }
      return { target: line, port: '' }
    })
...
async function confirmSave() {
  ...
  try {
    if (editingId.value) {
      ...
      const items = parseBatchTargets(form.value.targetsText || '')
      if (items.length === 0) {
        ElMessage.warning('请输入至少一个 IOC')
        return
      }
      await createTraced(items.map((item) => ({
        target: item.target,
        port: item.port,
        note: form.value.note,
      })))
      ElMessage.success(`已添加 ${items.length} 条备注`)
    }
